...
export const starsDataExportHandler = (db) => {
    return new Promise((resolve, reject) => {
        ...
        ipcRenderer.once(EVENTS.SHOW_CHOOSE_DIRECTORY_DIALOG_REPLG, (event, path) => {
            if (!path) {
            ...
            .then((data) => {
                ...
                ipcRenderer.send(EVENTS.EXPORT_STARS_DATA, JSON.stringify({path: filePath, data: JSON.stringify(data)}))
                ipcRenderer.once(EVENTS.EXPORT_STARS_DATA_SUCCESS_REPLY, () => {
                    resolve('data export successfully')
                })
                ipcRenderer.once(EVENTS.EXPORT_STARS_DATA_FAIL_REPLY, () => {
