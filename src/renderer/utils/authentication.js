...
export default class Authentication {
    static getLocalCredentials() {
        let promise = new Promise((resolve, reject) => {
            let username = localStorage.getItem(
                CONSTANTS.LOCAL_STORAGE_USERNAME_KEY
            );
            if (!username) {
                ...
                ipcRenderer.once(
                    ...
                    (event, arg) => {
                        !arg &&
                            reject(
                                new Error(
                                    "retrieve password from keychain failed"
                                )
                            );
    ...
    static saveCredentialsToSystem(credentials) {
        let promise = new Promise((resolve, reject) => {
            ...
            ipcRenderer.send(
                EVENTS.SAVE_CREDENTIALS_TO_SYSTEM,
                JSON.stringify(credentials)
            );
            ipcRenderer.once(
                EVENTS.SAVE_CREDENTIALS_TO_SYSTEM_REPLY,
                (event, result) => {
                    if (result) {
                        resolve(credentials);
                    } else {
                        reject(
                            new Error("save credentials to keychain failed")
                        );
                    }
                }
            );
    ...
    static deleteLocalCredentials() {
        let promise = new Promise((resolve, reject) => {
            ...
            if (!username) {
                ...
                ipcRenderer.once(
                    EVENTS.DELETE_LOCAL_CREDENTIALS_REPLY,
                    (event, result) => {
                        result
                            ? resolve(username)
                            : reject(
                                  new Error(
                                      "delete credentials from keychain failed"
                                  )
                              );
                    }
                );
        ...
        return githubClient
            ...
            .then(profile => {
                ...
                if (profile.login === credentials.username) {
                    ...
                    let p1 = Authentication.saveCredentialsToSystem(
                        credentials,
                        null
                    );
                    ...
                    let p3 = dbHandler
                        .initDB()
                        .then(() => dbHandler.upsertProfile(profile));
                    ...
                    return new Error(
                        "The token you provided does not match this account"
                    );
                }
            })
            .catch(err => {
