...
export default class DBHandler {
        ...
        if (conditions.group) {
            ...
            switch (conditions.group.type) {
                ...
                case CONSTANTS.GROUP_TYPE_CATEGORY:
                    ...
                    const category = await catsCollection
                        .findOne({ key: { $eq: id } })
                        .exec();
            ...
            switch (conditions.search.field) {
                ...
                case CONSTANTS.SEARCH_FIELD_REPO_TAGS:
                    const tag = await this.RxDB.tags
                        ...
                        .exec();
                    const tagRepoIds =
                        tag && tag.repos instanceof Array ? tag.repos : [];
                    if (args.id) {
                ...
                default:
                    ...
                    if (args.id) {
                        args.id.$in = Array.from(
                            new Set([].concat(args.id.$in, tempRepoIds))
                        );
                    } else {
    ...
    upsertOwners = async repos => {
        ...
        await authorsCollection
            .find({ isOwner: { $eq: true }, id: { $nin: ownerIds } })
            .remove();
    ...
    upsertContributors = async (repoId, contributors) => {
        ...
        const repo = await this.RxDB.repos
            .findOne({ id: { $eq: repoId } })
            .exec();
        if (!repo) {
            return false;
        }
    ...
    getRepoContributors = async repoId => {
        ...
        const authorsCollection = this.RxDB.authors;
        const docs = await authorsCollection
            .find({ repoId: { $eq: repoId } })
            .exec();
    ...
    upsertCategory = async name => {
        ...
        let docs = await catsCollection
            ...
            .exec();
        const start =
            docs instanceof Array && docs.length > 0 ? docs[0].id + 1 : 1;
        const date = new Date();
